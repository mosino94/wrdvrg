...
-- Example Basic RLS Policies (Frontend only reads its own data, writes handled by edge functions if needed or explicit RLS)
create policy "Users read own profile" on profiles for select using (id = auth.uid());
create policy "Users update own profile" on profiles for update using (id = auth.uid());

create policy "Users read own recovery codes" on recovery_codes for select using (profile_id = auth.uid());
create policy "Users read own trusted devices" on trusted_devices for select using (profile_id = auth.uid());
create policy "Users read own sessions" on sessions for select using (profile_id = auth.uid());

-- Presence is public read so users know who is online
create policy "Anyone read presence" on presence for select using (true);

-- Queue read own
create policy "Users read own queue" on queue for all using (profile_id = auth.uid());

-- Rooms read if participant
create policy "Users read own rooms" on rooms for select using (participant_1 = auth.uid() or participant_2 = auth.uid());
...
-- Callback requests
create policy "Users read own callback requests" on callback_requests for select using (requester_id = auth.uid() or target_id = auth.uid());

-- Call History
create policy "Users read own history" on call_history for all using (owner_id = auth.uid());

-- Blocks
create policy "Users read own blocks" on blocks for all using (blocker_id = auth.uid());

-- Reports (can see own reports)
create policy "Users read own reports" on reports for select using (reporter_id = auth.uid());

-- Moderation logs, recovery attempts, queue cooldowns are admin/service_role only or system reads.
-- For Edge Functions, we rely on service_role which bypasses RLS completely.
