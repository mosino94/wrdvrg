...
import { LogOut, MonitorSmartphone, Settings as SettingsIcon, Volume2, Shield } from 'lucide-react';
import { useAppStore } from '@/src/store/useAppStore';

export function Settings() {
  const { alias, countryCode, gender, isGuest, username, setAuthModalOpen, logout } = useAppStore();
  ...
  return (
    <div className="flex-1 w-full max-w-3xl mx-auto p-4 flex flex-col gap-8 h-full bg-[#080808] text-zinc-100 overflow-y-auto">
      ...
      <section className="bg-[#0C0C0C] border border-zinc-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-zinc-300">Profile Section</h2>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {countryCode && <img src={`https://flagcdn.com/24x18/${countryCode.toLowerCase()}.png`} alt={countryCode} className="w-6 h-4 object-cover rounded" />}
          ...
          <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
            ...
            <div className="flex items-center gap-2">
              <button className={cn("px-3 py-1.5 rounded-lg text-sm font-medium transition-all", gender === 'male' ? "bg-orange-500 text-white" : "bg-zinc-900 text-zinc-300 hover:bg-zinc-800")}>👨 Male</button>
              <button className={cn("px-3 py-1.5 rounded-lg text-sm font-medium transition-all", gender === 'female' ? "bg-orange-500 text-white" : "bg-zinc-900 text-zinc-300 hover:bg-zinc-800")}>👩 Female</button>
              <button className={cn("px-3 py-1.5 rounded-lg text-sm font-medium transition-all", gender === null ? "bg-orange-500 text-white" : "bg-zinc-900 text-zinc-300 hover:bg-zinc-800")}>🔒 Private</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#0C0C0C] border border-zinc-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-zinc-300 flex items-center gap-2">
          <Shield size={18} /> Privacy Section
        </h2>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
      ...
      <section className="bg-[#0C0C0C] border border-zinc-800 rounded-xl p-6">
        ...
        {isGuest ? (
          <div className="space-y-3">
            ...
            <button onClick={() => setAuthModalOpen(true)} className="w-full py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-medium transition-colors">
          ...
          <div className="space-y-3">
            ...
            <button onClick={logout} className="w-full py-3 bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-white rounded-lg font-medium flex items-center justify-center gap-2 transition-colors">
